...
const presentation = (route?: Route) => {
  ...
  if (route.route === "bypass") {
    ...
    return {
      label:
        relation?.relationship === "no_meaningful_addition"
...
const page = (args: {
  ...
  after: string;
  revision: number;
  terminalStatus: TurnStatus | undefined;
}): string => {
  const { turn, phase, turns, route, before, after, revision, terminalStatus } =
    args;
  ...
  const update =
    ...
    after.length > 0 &&
    after !== before;
  const summaryStatus = applied
    ? update
      ? `UPDATED · r${revision}`
      : terminalStatus === "no_update"
        ? "NO UPDATE"
        : terminalStatus === "retained" || terminalStatus === "budget_exceeded"
          ? "RETRY QUEUED"
          : `REVISION ${revision}`
    : decided
      ? route?.route === "bypass"
    ...
    for (let phase = 0; phase <= 3; phase += 1) {
      ...
      writeFileSync(
        ...
        page({
          ...
          revision,
          terminalStatus: snapshots.get(turn)?.statuses.at(-1),
        }),
      );
