...
const collectRecent = (recentRaw: RecentRaw, count?: number): Message[] => {
  ...
  const retained = isCollection(recentRaw) ? [] : (recentRaw.retainedFailures ?? []);
  const window = recentWindow(messages, count);
  const ids = new Set(window.map(({ id }) => id));
  return [...structuredClone([...retained]).filter(({ id }) => !ids.has(id)), ...window];
};
...
const compressedMemory = async (
  ...
  const candidate = MemoryPatchSchema.safeParse(
    await writer.compress({ memory: structuredClone(memory), taskContext, maxSummaryTokens }),
  );
  ...
  const topics = new Map(memory.topics.map((topic) => [topic.id, topic.version]));
  if (
...
export const renderContext = async (
  ...
  const taskContext = options.taskContext ?? { currentTask: "", compactionInstructions: [] };
  const summaryBudget = options.summaryBudgetTokens ?? DEFAULT_SUMMARY_TOKEN_BUDGET;
  const recent = collectRecent(recentRaw, options.recentMessageCount);
  const first = contentFor(memory, recent, taskContext);
  const warning = tokenizer.count(first.topics).tokens > summaryBudget * (budget.warningThreshold ?? DEFAULT_WARNING_THRESHOLD);
  const firstResult = budgetResult(first.content, budget, tokenizer, warning);
  if (firstResult.status === "rendered" || options.writer === undefined) return firstResult;

  let compressed: Memory | undefined;
  try {
    compressed = await compressedMemory(memory, taskContext, options.writer, summaryBudget);
  } catch {
    return firstResult;
  }
  if (compressed === undefined) return firstResult;
  const second = contentFor(compressed, recent, taskContext);
  const secondWarning = tokenizer.count(second.topics).tokens > summaryBudget * (budget.warningThreshold ?? DEFAULT_WARNING_THRESHOLD);
