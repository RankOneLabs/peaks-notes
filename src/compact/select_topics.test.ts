...
describe("selectTopics", () => {
  test("selects all qualifying topics", () => {
    const result = selectTopics(
      ...
      {
        topics: [
          { topicId: "a" as never, score: 0.7 },
          { topicId: "b" as never, score: 0.8 },
        ],
      },
      policy,
    );
    expect(result.ok && result.value.map(({ id }) => String(id))).toEqual([
      "a",
      "b",
    ]);
  });
  ...
  test.each([
    ...
    expect(selectTopics([topic("a")], response as never, policy).ok).toBe(
      false,
    );
  });
});
